...
function memeone_activate() 
{
	...
	update_option('memeone_version', 112);
}
...
{	
	...
	if(!isset($_POST['memeone_file_uploaded']) || $_POST['memeone_file_uploaded'] != 'Y'){

		// If not check if this is a request for meme
		if(!isset($_GET['meme'])){

			// If not, print the submission form
			$picture_message = 'Select a picture (.jpg, .png) Max size 2MB<font color="red">*</font>';
			$form = memeone_print_form($picture_message, $_POST['memeone_topline'], $_POST['memeone_bottomline']);
			return $form;
		
		// If this is a request for meme do the following
		}else{

			// Check if requested meme exists
			if (file_exists(get_option('memeone_default_upload_path').get_option('memeone_destination_folder').$_GET['meme'].'.jpg')) {

				// If so, display it
				return '<div class="memeone_new_meme_div"><img class="memeone_new_meme" src="'.get_option('memeone_default_upload_url').get_option('memeone_destination_folder').$_GET['meme'].'.jpg" /></div>'; 
			}else{ 

				// If not, display 404 meme
				return '<div class="memeone_new_meme_div"><img class="memeone_new_meme" src="'.plugins_url('memeone/images/404notfound.jpg').'" /></div>';
			}
		}
	// If this is a form submission, parse user input
	}else{
		
		// Check if picture is of a correct extension and size
		if($_FILES['memeone_picture']['tmp_name'] != "" && $_FILES['memeone_picture']['size'] < 2097152 && ($_FILES['memeone_picture']['type'] == 'image/jpeg' || $_FILES['memeone_picture']['type'] == 'image/png')){
			$picture = $_FILES['memeone_picture']['tmp_name'];
		}else{
			$hasError = true;
			$picture_message = '<font color="red"><b>Select a picture (.jpg, .png) Max size 2MB*</b></font>';
		}

		// If everything there is an error, display error message and print the form again
		if($hasError != ""){ 
			$form = memeone_print_form($picture_message, $_POST['memeone_topline'], $_POST['memeone_bottomline']);
			return $form;

		// If no erros were found, create a meme	
		}else{ 
			$topline = $_POST['memeone_topline'];
			$bottomline = $_POST['memeone_bottomline'];

			$meme_name = memeone_create_meme($topline, $bottomline, $picture);
			// Add a record to db about our new meme
			memeone_add_db_record($topline, $bottomline, $meme_name);

			/*
			*	Since we can not implement 'post redirect get' the 'php way' at this point (headers are already sent)
			*	we will do a js workaround. This is done to prevent data resubmission on page refresh.
			*/
			memeone_post_redirect_get('meme='.$meme_name);
		}	
	}
}
